...
| Scoped Proposal & Project Context | Supporting data flow: the orchestrator searches eligible project data across the system and across semesters, then applies permission, candidate-state, privacy and field-allowlist controls before use. Only a bounded relevant candidate set is sent to the analysis provider; the entire EHUB database is never exposed. |
