...
const ICON_ASSIGNMENTS = {
  ...
  'notification-channel': 'bell', 'signalr-push': 'radio', 'email-channel': 'mail', 'failure-monitor': 'alert', 'scale-note': 'server',
};
...
const diagrams = [
  ...
  },
  {
    ...
    groups: [
      group('realtime-lane', 25, 140, 1630, 390, 'Authorized Realtime Messaging Path', 'frontend', { compactTitleGap: true }),
      group('async-lane', 25, 560, 1630, 390, 'Transactional Outbox and Delivery Path', 'backend', { compactTitleGap: true }),
    ],
    nodes: [
      node('rt-user', 50, 220, 140, 135, 'EHub User', ['Authorized role'], 'actor'),
      node('signalr-client', 230, 220, 175, 135, 'React SignalR Client', ['JWT connection and reconnect'], 'frontend'),
      node('signalr-hub', 450, 220, 180, 135, 'SignalR Hub', ['Authenticate connection and receive commands'], 'backend'),
      node('membership-auth', 675, 220, 190, 135, 'Membership Authorization', ['Class/team scope and active membership'], 'security'),
      node('message-service', 910, 220, 200, 135, 'Chat Message Service', ['Validate, timestamp and persist before push'], 'domain'),
      node('rt-database', 935, 395, 150, 105, 'PostgreSQL', ['Messages and memberships'], 'data'),
      node('broadcast', 1450, 220, 170, 135, 'Group Broadcast', ['Best-effort message and presence push'], 'success'),
      node('scale-note', 300, 395, 170, 105, 'Optional Redis Backplane', ['Required only for multiple API instances'], 'infra'),

      node('business-action', 55, 700, 190, 125, 'Business Transaction', ['Class, team or mentor change'], 'domain'),
      node('outbox-store', 290, 700, 210, 125, 'Atomic State and Outbox', ['Business data and pending event'], 'data'),
      node('outbox-worker', 545, 700, 190, 125, 'Outbox Worker', ['Lease, retry and idempotency'], 'backend'),
      node('event-dispatcher', 780, 700, 200, 125, 'Event Dispatcher', ['Route by versioned event type'], 'backend'),
      node('failure-monitor', 545, 850, 190, 80, 'Failure Monitoring', ['Terminal failure and alert'], 'danger'),
      node('membership-sync', 1030, 700, 185, 105, 'Chat Membership Sync', ['Idempotent grant and revoke'], 'security'),
      node('notification-channel', 790, 835, 180, 90, 'Notification Projection', ['Persist an idempotent in-app record'], 'backend'),
      node('email-channel', 1255, 590, 180, 95, 'Email Channel', ['Optional external delivery'], 'external'),
      node('signalr-push', 1460, 700, 165, 105, 'SignalR Push', ['Access changes and unread count'], 'success'),
    ],
    edges: [
      edge('r1', 'rt-user', 'signalr-client', 'open application', { sourceSide: 'right', targetSide: 'left' }),
      edge('r2', 'signalr-client', 'signalr-hub', 'authenticated SignalR connection', { sourceSide: 'right', targetSide: 'left' }),
      edge('r3', 'signalr-hub', 'membership-auth', 'authorize group and command', { sourceSide: 'right', targetSide: 'left' }),
      edge('r4', 'membership-auth', 'message-service', 'allowed command', { sourceSide: 'right', targetSide: 'left' }),
      edge('r5', 'message-service', 'rt-database', 'persist before push', { sourceSide: 'bottom', targetSide: 'top' }),
      edge('r6', 'message-service', 'broadcast', 'broadcast only after commit', { sourceSide: 'right', targetSide: 'left' }),
      edge('r7', 'broadcast', 'signalr-client', 'push to group', {
        sourceSide: 'top', targetSide: 'top', dashed: true,
        points: [{ x: 1535, y: 190 }, { x: 317, y: 190 }], labelX: 930, labelY: 184,
      }),
      edge('r8', 'business-action', 'outbox-store', 'same PostgreSQL transaction', { sourceSide: 'right', targetSide: 'left' }),
      edge('r9', 'outbox-store', 'outbox-worker', 'claim pending with lease', { sourceSide: 'right', targetSide: 'left' }),
      edge('r10', 'outbox-worker', 'event-dispatcher', 'idempotent dispatch', { sourceSide: 'right', targetSide: 'left' }),
      ...
      edge('r13', 'event-dispatcher', 'email-channel', 'optional email event', {
        sourceSide: 'top', targetSide: 'top', points: [{ x: 880, y: 575 }, { x: 1345, y: 575 }], labelX: 1115, labelY: 569,
      }),
      edge('r14', 'outbox-worker', 'failure-monitor', 'retry limit reached', {
        sourceSide: 'bottom', targetSide: 'top', dashed: true,
      }),
      edge('r15', 'membership-sync', 'signalr-push', 'access update', {
        sourceSide: 'right', targetSide: 'left', dashed: true,
      }),
      edge('r16', 'notification-channel', 'signalr-push', 'notification update', {
        sourceSide: 'right', targetSide: 'bottom', dashed: true,
        points: [{ x: 1542.5, y: 872 }], labelX: 1240, labelY: 866,
      }),
      edge('r17', 'signalr-push', 'signalr-hub', 'publish realtime update', {
        sourceSide: 'top', targetSide: 'bottom', targetRatio: 0.5, dashed: true,
        points: [{ x: 1542.5, y: 540 }, { x: 540, y: 540 }], labelX: 1100, labelY: 534,
      }),
      edge('r18', 'signalr-hub', 'scale-note', 'optional scale-out backplane', {
        sourceSide: 'bottom', sourceRatio: 0.2, targetSide: 'top', dashed: true,
        points: [{ x: 486, y: 380 }, { x: 385, y: 380 }], labelX: 475, labelY: 396,
      }),
    ],
  },
];
...
const COMPACT_BODIES = {
  ...
  broadcast: 'Best-effort group push',
  'business-action': 'State + Event',
  'outbox-store': 'One PostgreSQL transaction',
  'outbox-worker': 'Lease • Retry • Idempotency',
  'event-dispatcher': 'Versioned event routing',
  'membership-sync': 'Idempotent grant • revoke',
  'notification-channel': 'Idempotent in-app record',
  'signalr-push': 'Access • Unread count',
  'email-channel': 'Optional delivery',
  'failure-monitor': 'Terminal failed • Alert',
  'scale-note': 'Multiple API instances only',
};
...
const COMPACT_EDGE_LABELS = {
  ...
  r17: 'Realtime Update', r18: 'Scale-out Only',
};
...
const SHORT_TITLES = {
  ...
  'outbox-store': 'Atomic State & Outbox',
  'outbox-worker': 'Outbox Worker',
  'event-dispatcher': 'Event Dispatcher',
  'membership-sync': 'Membership Sync',
  'notification-channel': 'Notification Projection',
  'signalr-push': 'SignalR Push',
  'email-channel': 'Email Channel',
  'failure-monitor': 'Failure Monitoring',
  'scale-note': 'Optional Redis Backplane',
};
