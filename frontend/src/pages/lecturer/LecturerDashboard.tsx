        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }} className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
            ...
            {myClasses.length === 0 ? (
              ...
              <div className="space-y-3">
                {myClasses.map(cls => (
                  <div key={cls._id || cls.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 hover:border-primary-200 hover:bg-primary-50/30 transition-all cursor-pointer" onClick={() => navigate(`/classes/${cls.slug || cls._id || cls.id}`)}>
